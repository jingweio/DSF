...
class DSF_GPR_R(torch.nn.Module):
    def __init__(self, in_dim, hid_dim, out_dim, args, PE_in_dim, PE_hid_dim, PE_alpha, K=10):
        super(DSF_GPR_R, self).__init__()
        ...
        self.prop1 = _prop(K, args.alpha, args.Init, PE_hid_dim=PE_hid_dim, PE_alpha=PE_alpha)
...
class _prop(MessagePassing):
    def __init__(self, K, alpha, Init, PE_hid_dim, PE_alpha, **kwargs):
    ...
    def forward(self, x, edge_index, edge_weight=None, pe=None):
        ...
        for k in range(self.K):
            ...
            pe = self.PE_alpha * raw_pe + (1 - self.PE_alpha) * self.propagate(edge_index, x=pe, norm=norm)
            pe = torch.tanh(pe)
